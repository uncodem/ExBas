let calc_labels emit_buffer =
    ...
    let aux acc = function
        ...
        | _ -> acc + 1
    in
    let _ = List.fold_left aux 0 emit_buffer in
    tbl

let rec emit_binary labels acc pos = function
    | [] -> List.rev acc
    | Emitter.RawOp op :: rest ->
        emit_binary labels (Opcodes.int_of_exbvm_op op :: acc) (pos + 1) rest
    | Emitter.RawValB x :: rest ->
        emit_binary labels ((x land 0xff) :: acc) (pos + 1) rest
    ...
    | Emitter.LabelRef name :: rest ->
        ...
        emit_binary labels (h :: l :: acc) (pos + 2) rest
    | Emitter.LabelDef _ :: rest | Emitter.NoEmit :: rest ->
        emit_binary labels acc pos rest
...
let emit_final emitted const_pool =
    ...
    let header =
        [ 0xef; 0xbe; 0xad; 0xde ] @ int_to_le_bytes (List.length binary) 4
    in
